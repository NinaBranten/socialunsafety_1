...
<br><br> **Overall unsafety (class 6)**


## Regression overall unsafety
...
#ICC
0.1494/(0.1494+115.2260)
#PCV
(0.8319-0.1494)/0.8319
```
...
```{r top/bottom 10 overall unsafety model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
pred_df_6 <- NEA2022 %>%
  mutate(
...
<br><br> **Supervisor unsafety (class 6)**
...
#ICC
0.08421/(0.08421+132.09111)
#PCV
(0.1575-0.08421)/0.1575
```

```{r estimated supervisor unsafety, cache=TRUE}
#Predicted probability belonging to supervisor unsafety class
...
```{r top/bottom 10 supervisor unsafety model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
...
<br><br> **Coworker unsafety (class 7)**
...
```{r estimated probability coworker unsafety, cache=TRUE}
#Predicted probability belonging to coworker unsafety class
...
```{r top/bottom 10 coworker unsafety model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
pred_df_7 <- NEA2022 %>%
  mutate(
...
<br><br> **Unsafe social climate (class 5)**
...
tab_model(L_5_0, L_5_1, p.style="stars")

#ICC
0.1884/(0.1884+587.6543)
#PCV
(3.81-0.1884)/3.81
```
...
#Predicted probability belonging to unsafe social climate class
...
```{r top/bottom 10 unsafe social climate model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
pred_df_5 <- NEA2022 %>%
  mutate(
...
<br><br> **Short-term conflict (class 1)**
...
#ICC
0.8462/(0.8462+435.4613)
#PCV
(1.791-0.8462)/1.791
```

```{r estimated short-term conflict, cache=TRUE}
#Predicted probability belonging to short-term conflict class
...
```{r top/bottom 10 short-term conflict model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
...
#ICC
2.092/(2.092+1448.047)
#PCV
(3.552-2.092)/3.552
```

```{r estimated safe, cache=TRUE}
#Predicted probability belonging to safe class
...
```{r top/bottom 10 safe model0, cache=TRUE, results='hide'}
#Top/bottom 10 based on model0
...
#ICC
2.231/(2.231+1254.840)
#PCV
(15-2.231)/15
```

```{r estimated highly safe, cache=TRUE}
#Predicted probability belonging to highly safe class
...
```{r top/bottom 10 highly safe model0, cache=TRUE}
#Top/bottom 10 based on model0
