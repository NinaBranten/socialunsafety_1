...
# Step 1: Extract posterior probabilities (N x K matrix)
posterior <- model8s3_29923$posterior

# Step 2: Compute individual entropy contributions
entropy_individual <- -rowSums(posterior * log(posterior + 1e-10))  # add small value to avoid log(0)

# Step 3: Calculate total (normalized) entropy
N <- nrow(posterior)
K <- ncol(posterior)
entropy_total <- 1 - (sum(entropy_individual) / (N * log(K)))
