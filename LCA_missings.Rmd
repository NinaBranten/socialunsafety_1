...
```{r}
datam <- data.frame(pesten_col, pesten_leid, intimidation_col, intimidation_leid, geweld_col, geweld_leid, seks_col, seks_leid, conflict_col, conflict_leid, conflict_werk, steunleid_welzijn, steunleid_aandacht, steuncol_belang, steuncol_vriendelijk, psyveil_last, psyveil_hulp, psyveil_fout)

itemsm <- cbind(pesten_col, pesten_leid, intimidation_col, intimidation_leid, geweld_col, geweld_leid, seks_col, seks_leid, conflict_col, conflict_leid, conflict_werk, steunleid_welzijn, steunleid_aandacht, steuncol_belang, steuncol_vriendelijk, psyveil_last, psyveil_hulp, psyveil_fout) ~ 1
```
<br><br> Variabelen:
...
```{r, cache=TRUE}
model13m <- poLCA(itemsm, datam, nclass = 13, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```

```{r, cache=TRUE}
model14m <- poLCA(itemsm, datam, nclass = 14, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```

```{r, cache=TRUE}
model15m <- poLCA(itemsm, datam, nclass = 15, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```

```{r, cache=TRUE}
model16m <- poLCA(itemsm, datam, nclass = 16, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```

```{r, cache=TRUE}
model17m <- poLCA(itemsm, datam, nclass = 17, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```

```{r, cache=TRUE}
model18m <- poLCA(itemsm, datam, nclass = 18, maxiter = 5000, nrep = 10, graphs=FALSE, na.rm = TRUE)
```
